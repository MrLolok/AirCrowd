...
public interface FlightFlowConditionProvider extends ConditionProvider {
    class FlightsDataExtractor {
        private final static Logger LOGGER = LogManager.getLogger(FlightsDataExtractor.class);

        public static FlightFlowData extract(List<Row> rows) {
            List<FlightData> flightDataList = rows.stream().map(FlightFlowConditionProvider.FlightsDataExtractor::extract).toList();
        ...
        public static FlightData extract(Row row) {
            ...
            LocalDateTime datetime = row.getTimestamp(row.fieldIndex("datetime")).toLocalDateTime();
